...
BRACKET_SET_INTERVAL = 3        # seconds between each set of 3 captures
BRACKET_SHOT_DELAY = 3            # seconds between each shot within a set
BRACKET_SAVE_DIR = os.path.expanduser("~/mef_captures")
BRACKET_JPEG_QUALITY = 95         # higher quality for dataset images

# 3 fixed shutter speeds in microseconds: under, normal, over
BRACKET_SHUTTER_SPEEDS = [
    5000,       # LOW  exposure  — dark/fast  (≈1/2000s)
    33000,     # NORMAL         — daylight   (≈1/30s)
    500000,   # HIGH exposure  — bright     (2s)
]
# AnalogueGain per bracket shot (None = keep auto gain, float = manual)
BRACKET_GAINS = [
    None,      # LOW  — use current gain
    None,      # NORMAL — use current gain
    10.0,       # HIGH — boost gain for overexposed shot
]
...
def capture_bracket_set(cam):
    ...
    # Lock current gain so it doesn't drift between shots
    current_gain = metadata.get("AnalogueGain", 1.0)

    for i, shutter_us in enumerate(BRACKET_SHUTTER_SPEEDS):
        if not running:
            break

        # Clamp to HQ camera hardware limits (µs)
        target_exposure = max(303, min(shutter_us, 6_000_000))

        # Set shutter speed + optional gain boost
        controls = {"ExposureTime": target_exposure}
        if BRACKET_GAINS[i] is not None:
            controls["AnalogueGain"] = BRACKET_GAINS[i]
        else:
            controls["AnalogueGain"] = current_gain
        cam.set_controls(controls)

        # Skip frames so the sensor applies the new exposure
        # Fewer settle frames for long exposures (each frame takes shutter_us to capture)
        settle = 1 if shutter_us > 100000 else BRACKET_SETTLE_FRAMES
        ...
        if i < len(BRACKET_SHUTTER_SPEEDS) - 1:
            for _ in range(int(BRACKET_SHOT_DELAY * 10)):
                if not running:
                    break
                time.sleep(0.1)

    # Reset gain to auto level before re-enabling AE (avoids bright flash)
    cam.set_controls({"AnalogueGain": current_gain, "ExposureTime": current_exposure})
    time.sleep(0.1)
